...
func TestDistance2DLinePointAngular(t *testing.T) {
	...
	l.P.X, l.P.Y = l.P.X+l.V.X, l.P.Y+l.V.Y
	l.V.X, l.V.Y = -l.V.X, -l.V.Y
...
func Benchmark_Distance2D_LinePointAngular(b *testing.B) {
	l, p := &Line2D{Vector2D{1, 1}, Vector2D{-1, -1}}, &Vector2D{1, 0.5}
	for i := 0; i < b.N; i++ {
...
func TestDistance2DLinePoint(t *testing.T) {
	l, p := &Line2D{Vector2D{}, Vector2D{1, 0}}, &Vector2D{0, 1}
...
func TestDistance2DLineSegmentPoint(t *testing.T) {
	l := &Line2D{Vector2D{0, 1}, Vector2D{1, 0}}
	p := &Vector2D{-1, 0}
